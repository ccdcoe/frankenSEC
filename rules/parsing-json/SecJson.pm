...
our $VERSION = 1.00;
our @EXPORT_OK = qw(json2matchvar alertajson);
...
# An example function for generating JSON records for Alerta.
# This function is invoked by SEC 'lcall' actions with :> operator.
...
sub alertajson {
  ...
  if (!exists($json{"text"})) {
    $json{"text"} = $main::variables{"text"};
  }

  if (!exists($json{"event"})) {
    $json{"event"} = $main::variables{"event"};
  }

  if (!exists($json{"value"})) {
    $json{"value"} = $main::variables{"value"};
  }

  $json{"correlate"} = [ $json{"event"} ];
#  $json{"service"} = [ $json{"syslog_program"} ];
#  $json{"tags"} = []; 
#  $json{"group"} = undef;
#  $json{"timeout"} = 900;
#  $json{"timeout"} += 0;
#  $json{"rawData"} = undef; 
  ...
  return JSON->new->utf8->allow_unknown->encode(\%json);
}
